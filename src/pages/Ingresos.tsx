...
import { Plus, Filter, Trash2, Search, ChevronDown, Check } from 'lucide-react';
import Modal from '../components/Modal';
import StatusBadge from '../components/StatusBadge';
import { tiposGalon } from '../lib/mockData';
import { useApp } from '../lib/AppContext';
import { formatDateTime, formatNumber, padFicha } from '../lib/utils';
import type { FichaIngreso, FichaItem } from '../lib/types';

type EstadoFiltro = '' | 'pendiente' | 'recibido' | 'rechazado';
...
const TRANSICIONES_INGRESO: Record<string, FichaIngreso['estado'][]> = {
  pendiente: ['recibido', 'rechazado'],
  recibido:  [],
  rechazado: [],
};

const LABELS_INGRESO: Record<string, string> = {
  recibido:  'Marcar Recibido',
  rechazado: 'Marcar Rechazado',
};

function DropdownEstadoIngreso({
  id, estado, onChange,
}: { id: string; estado: FichaIngreso['estado']; onChange: (id: string, e: FichaIngreso['estado']) => void }) {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  const siguientes = TRANSICIONES_INGRESO[estado] ?? [];
  ...
  return (
    <div className="relative inline-block" ref={ref}>
      ...
      {open && (
        <div
          className="absolute left-0 top-full mt-1 z-30 rounded-lg overflow-hidden shadow-xl min-w-[160px]"
          style={{ background: '#1e2432', border: '1px solid #2d3748' }}
        >
          {siguientes.map(s => (
            <button key={s} onClick={() => { onChange(id, s); setOpen(false); }}
              className="flex items-center gap-2 w-full px-3 py-2 text-xs text-left hover:bg-[#252b3b] transition-colors">
              <Check size={11} style={{ opacity: 0 }} />
              <StatusBadge estado={s} small />
              <span style={{ color: '#8892a4' }}>{LABELS_INGRESO[s]}</span>
            </button>
          ))}
        </div>
      )}
...
export default function Ingresos() {
  const { fichasIngreso: records, cambiarEstadoIngreso, addIngreso } = useApp();
  ...
  function cambiarEstado(id: string, nuevo: FichaIngreso['estado']) {
    cambiarEstadoIngreso(id, nuevo);
  }

  const filtered = useMemo(() => records.filter(r => {
    if (statusFilter && r.estado !== statusFilter) return false;
    if (searchDate && !r.fecha_ingreso.startsWith(searchDate)) return false;
    return true;
  ...
  function validate() {
    ...
    form.items.forEach((it, i) => {
      if (!it.cantidad || parseInt(it.cantidad) <= 0) e[`item_${i}`] = 'Debe ser > 0';
    });
  ...
  function handleSubmit(ev: React.FormEvent) {
    ...
    setTimeout(() => {
      ...
      const nueva: FichaIngreso = {
        id: `i${Date.now()}`,
        numero_ficha: padFicha('ING', records.length + 1),
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">Fichas de Ingreso</h1>
      ...
      <div className="flex flex-wrap gap-3 p-4 rounded-xl" style={{ background: '#1e2432', border: '1px solid #2d3748' }}>
        <div className="flex items-center gap-2 flex-1 min-w-[180px]">
          ...
          <input
            ...
            className="bg-transparent text-sm outline-none w-full" style={{ color: '#e2e8f0' }}
          />
        </div>
        ...
        {(searchDate || statusFilter) && (
          <button onClick={() => { setSearchDate(''); setStatusFilter(''); }} className="text-xs px-3 py-1 rounded" style={{ color: '#f87171', background: '#450a0a' }}>
            Limpiar
          </button>
        )}
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: '1px solid #2d3748', background: '#171c28' }}>
                {['N° Ficha', 'Fecha Ingreso', 'Proveedor', 'Cilindros', 'Total', 'Estado', 'Observaciones'].map(h => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-10 text-center text-sm" style={{ color: '#4a5568' }}>No se encontraron registros</td></tr>
              ) : filtered.map(f => (
                <tr key={f.id} style={{ borderBottom: '1px solid #1a1f2e' }}
                  ...
                  <td className="px-4 py-3 text-xs whitespace-nowrap" style={{ color: '#8892a4' }}>{formatDateTime(f.fecha_ingreso)}</td>
                  <td className="px-4 py-3 text-xs text-slate-300 max-w-[130px]">
                    <span className="truncate block">{f.proveedor.split(' — ')[0]}</span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <DropdownEstadoIngreso id={f.id} estado={f.estado} onChange={cambiarEstado} />
                  </td>
                  <td className="px-4 py-3 text-xs max-w-[150px] truncate" style={{ color: '#8892a4' }}>{f.observaciones || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Modal */}
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Nueva Ficha de Ingreso">
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Proveedor */}
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: '#8892a4' }}>Proveedor *</label>
            <input
              type="text" value={form.proveedor}
              onChange={e => setForm(p => ({ ...p, proveedor: e.target.value }))}
              placeholder="Ej: Planta Concepción — Abastible"
              className="w-full rounded-lg px-3 py-2.5 text-sm outline-none"
              style={{ background: '#111827', border: `1px solid ${errors.proveedor ? '#ef4444' : '#2d3748'}`, color: '#e2e8f0' }}
            />
            {errors.proveedor && <p className="text-xs mt-1" style={{ color: '#f87171' }}>{errors.proveedor}</p>}
          </div>
          ...
          <div>
              ...
              {form.items.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex gap-2 items-center">
                    ...
                    <select
                      value={item.tipo_galon_id}
                      onChange={e => updateItem(idx, 'tipo_galon_id', e.target.value)}
                    ...
                    <input
                      ...
                      style={{ background: '#111827', border: `1px solid ${errors[`item_${idx}`] ? '#ef4444' : '#2d3748'}`, color: '#e2e8f0' }}
                    />
                    {form.items.length > 1 && (
            ...
            {previewTotal > 0 && (
              <div className="mt-2.5 flex justify-between items-center text-xs px-3 py-2 rounded-lg" style={{ background: '#111827' }}>
                <span style={{ color: '#4a5568' }}>Total a registrar</span>
